...
class Faculty:
    ...
    def to_html_row(self):
        return f"""
        <tr>
            <td>{html.escape(self.fname)}{html.escape(self.mi)}{html.excape(self.lname)}</td>
...
class FacultyDirectory:
    ...
    def get_faculty(self,search=None,sort=None):
        ...
        if sort in self.allowed_sorts:
            query +=f" ORDER BY {self.allowed_sortrs[sort]}"
   ...
   @staticmethod 
   def list_to_html(self,faculty_list):
...
facutly_members = directory.get_faculty(search=lname, sort=sort)
